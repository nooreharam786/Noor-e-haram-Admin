    ...
    content: [
      ...
      sectionLabel("Participant details"),
      {
        table: {
          widths: ["*", "*"],
          body: [
            [detailCell("Participant name", applicant.user.name), detailCell("Mobile number", applicant.phone)],
...
function chitCell(item: PrintApplicant) {
  return {
    stack: [
      ...
      { text: shortText(item.user.name, 40), fontSize: 8.5, bold: true, color: brand.ink, alignment: "center", noWrap: true, margin: [4, 0, 4, 2] },
...
export function downloadParticipantCsv(items: PrintApplicant[]) {
  ...
  const rows = items.map((item) => [item.registrationNo ?? "", item.user.name, item.phone, formatDate(item.createdAt, true), item.status.replace("_", " ")]);
