...
export function pdfTopHeader(title: string = "Official Lucky Draw Registration Ticket", logoUrl?: string): string {
  ...
  return `
    <div style="display: flex; flex-direction: column; align-items: center; margin-bottom: 20px; padding-top: 10px;">
      <img src="${logoSrc}" style="height: 65px; max-width: 220px; object-fit: contain; margin-bottom: 12px; display: block;" onerror="this.src='/noor-e-haram-logo3.png'" />
      <h1 class="pdf-serif" style="font-size: 24px; font-weight: 700; color: ${PDF_COLORS.primary}; margin: 0 0 6px 0; letter-spacing: 0.05em; text-transform: uppercase; line-height: 1.2;">
        NOOR E HARAM
      </h1>
      <div style="display: flex; align-items: center; justify-content: center; gap: 12px; margin: 4px 0 8px 0; width: 100%;">
        <div style="width: 40px; height: 1px; background-color: ${PDF_COLORS.gold}; flex-shrink: 0;"></div>
        <span style="font-size: 11px; font-weight: 600; color: ${PDF_COLORS.gold}; text-transform: uppercase; letter-spacing: 0.15em; white-space: nowrap;">CHARITY FOUNDATION</span>
        <div style="width: 40px; height: 1px; background-color: ${PDF_COLORS.gold}; flex-shrink: 0;"></div>
      </div>
      <p style="font-size: 10px; color: ${PDF_COLORS.textMain}; margin: 2px 0 14px 0; font-weight: 500; letter-spacing: 0.05em;">Faith • Service • Humanity</p>

      <h2 style="font-size: 15px; font-weight: 700; color: ${PDF_COLORS.primary}; margin: 4px 0 0 0; text-transform: uppercase; letter-spacing: 0.05em;">
        ${title}
      </h2>
    </div>
  `;
...
export function pdfSealsRow(customSealUrl?: string, customSignatureUrl?: string, signatoryName?: string): string {
  ...
  const sealContent = customSealUrl
    ...
    : `<svg viewBox="0 0 100 100" width="100%" height="100%">
        <circle cx="50" cy="50" r="48" fill="none" stroke="${PDF_COLORS.gold}" stroke-width="1.5" stroke-dasharray="3,3"/>
        <circle cx="50" cy="50" r="43" fill="none" stroke="${PDF_COLORS.gold}" stroke-width="0.75"/>
        <path id="curve" fill="transparent" d="M 14,50 A 36,36 0 1,1 86,50 A 36,36 0 1,1 14,50" />
        <text fill="${PDF_COLORS.gold}" font-size="7.5" font-weight="bold" letter-spacing="0.8">
          <textPath href="#curve" startOffset="50%" text-anchor="middle">NOOR E HARAM CHARITY FOUNDATION</textPath>
        </text>
        <rect x="36" y="32" width="28" height="32" fill="${PDF_COLORS.gold}" rx="2"/>
        <rect x="42" y="27" width="16" height="5" fill="${PDF_COLORS.gold}" rx="1"/>
        <text x="14" y="53" font-size="10" fill="${PDF_COLORS.gold}">★</text>
        <text x="76" y="53" font-size="10" fill="${PDF_COLORS.gold}">★</text>
      </svg>`;
